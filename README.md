...
Optional: [GoCDKeys comparison replies](docs/GOCDKEYS.md) add a compact, validated price-comparison button to resolvable Instant Gaming/DealGecko game titles in gaming-deals (paid/reduced deals), including embed-only posts without price text. Disabled by default; free-games is handled separately and intentionally receives no comparisons. Owner/admin `/deals backfill` previews the last 25/50/100 messages (default 50), then adds missing comparisons only after confirmation, sharing live delivery claims.
