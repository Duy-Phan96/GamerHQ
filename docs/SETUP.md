...
Paid deals in gaming-deals can receive optional GoCDKeys comparisons from Instant Gaming or DealGecko; free-games is always excluded. The managed gaming-deals channel defines the paid/reduced scope; a recognized positive price is not required. Health checks GamerHQ posting permissions and the local intent. Enable the existing GOCDKEYS_ENABLED setting and Message Content Intent after reviewing [paid-deal setup](GOCDKEYS.md). Route free and paid posts separately in DealGecko. For existing posts, run `/deals backfill`, choose Preview, then Run Backfill. The default scans 50 messages, optionally 25 or 100. Already handled/uncertain deliveries are retained. Run `/server health` and owner `/server setup` → Repair to fix scoped Instant Gaming/Gaming Bots parent discovery and gaming-deals posting; reopen the external Instant Gaming `/config` to verify selection. Private access and free-games routing remain unchanged.
